...
namespace _CsharpEgitimKapmi301_EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        private void FrmStatistics_Load(object sender, EventArgs e)
        {
            ...
            lblAvgPrice.Text = db.Location.Average(x => x.Price).ToString() + "₺";

            int lastCountryId = db.Location.Max(x => x.Id);
            lblLastCountry.Text = db.Location.Where(x => x.Id == lastCountryId).Select(y => y.Country).FirstOrDefault();
            ...
            lblMaxCapacityLocation.Text = db.Location.Where(y => y.Capacity == maxCapacity).Select(y => y.City).FirstOrDefault().ToString();
